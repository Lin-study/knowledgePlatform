...
const formatterPath = (p) => path.join(__dirname, '..', p)

const getdir = (dir = '_book', list = [], parentName) => {
  var files = fs.readdirSync(formatterPath(dir)).reverse();
  files.forEach(function(filename) {
    if (filterName.includes(filename)) return
    var filePath = path.join(dir, filename);
    var stats = fs.statSync(formatterPath(filePath));
    ...
    list.push(item)
    if (isDirectory && !filterPath.find(p => filePath.includes(p))) {
...
const getBook = (bookPath) => {
  return fs.readFileSync(formatterPath(bookPath))
}
